...
## Abstract
This analysis deals with exploring different morphometric features of cell nuclei and their relationship to diagnosis. The key morphometric features that we are going to focus on are the radius, perimeter, area, smoothness, compactness, concavity, and the number of concave portions. Within each section of the analysis, we will explore each feature separately in relation to diagnosis (benign vs malignant) and then compare each feature to each other. The data represented in this analysis can be found on [archive.ics.uci.edu](https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Wisconsin+%28Diagnostic%29).
...
Smoothness is calculated by the local variation in radius lengths within a cell nuclei. The closer it is to zero, the less variation and more smooth the cell nuclei is. The further away it is from zero, the more variability and less smooth the cell nuclei is. 
...
Compactness is defined by the cells ability to be packed together closely.
...
* The closer you are to zero the higher the likelihood you have a benign cell nuclei and the further you are from zero the higher the likelihood you have a malignant cell nuclei.
...
## Concave Severity & Amount to Cell Nuclei Diagnosis (Benign vs Malignant)

Within this section of the analysis, concavity represents the severity of concave portions of the contour and the concave points represents the number of concave portions of the contour. The larger the number, the more severe the concave portions are and the higher the number of concave portions within a contour.
...
```
The range of the data within concavity is between 0 and 0.427. Zero indicating no severity in concave portions (relatively flat).

Let's see if we can visualize the relationship between concavity (severity of concave portions) to diagnosis:
```{r}
...
ggplot(mean_bc_data, aes(x = diagnosis, y = concave.points_mean)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
```
The higher the number of concave points (or the higher the percent of the cell nuclei with concave portions), the higher the likelihood of malignancy.
Both the severity and amount of concave points indicate a positive relationship with diagnosis.
...
ggplot(mean_bc_data, aes(x = concave.points_mean, y = concavity_mean, color = diagnosis)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
```
This relationship makes sense because the higher the number or proportion of concave points within a cell nuclei, the higher the severity.
...
* 100% of cancer cells with an average number of concave Portions > 0.0853 are Malignant!
...
With that being said, can we use number of concave points as an strong indicator/predictor for malignancy within a cell nuclei?
such as that an average greater than .0853 is a good indicator of malignancy? Can this be applied to the population?

* We can explore that with Logistic Linear Regression and see if this variable is a good predictor of malignancy compared to the other known variables.

## Logistic Regression - Binomial for all Analyzed Variables in Relation to Diagnosis

Below is a Binomial Logistic Linear Regression for predicting the diagnosis of the cell nuclei. 

The model will utilize the variables used throughout this analysis (radius, perimeter, area, smoothness, compactness, concavity, and number of concave points) and determine which are good predictors and which are not. Good predictors are indicated by a small p-value and large effect size (Estimate).
...
Now we will look at the effect size (Estimate). Although the average area has a statistically significant p-value, the effect size is relatively small at 0.03258.
...
As seen by the graph, our linear model has accurately captured the relationship of the variables to diagnosis.

## Conclusion

One of the most common cancers within the female population is breast carcinoma. Understanding the correlation between the nuclear morphometry and diagnosis can lead to early and accurate diagnosis. Which can lead to early treatment. As shown in this analysis, the radius, perimeter, area, smoothness, compactness, concavity, and the number of concave portions all showed a positive correlation with malignancy. As the number grew further from zero, the higher the likelihood of the cell nuclei being malignant. 

Although all morphometric features showed statistically meaningful correlations, there were only a select few that were marked as good predictors of malignancy as seen in our Binomial Linear Regression model. Overall, the average area and number of concave points of the cell nuclei were the two good predictors of malignancy in our analysis. The reason being, the average area had a p-value of 0.013 and an effect size of 0.033 and the average number of concave points had a p-value of 0.0043 and an effect size of 66.82. Due to the large effect size and small p-value, the average number of concave points is the best predictor within our analysis.
